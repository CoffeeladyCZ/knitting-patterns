...
import type { CreateIssueResult, CreateIssueVariables, RepositoriesData, RepositoriesVariables, RepositoryIssuesData, RepositoryIssuesVariables } from "./types";
...
export const useViewerRepositories = (limit: number) => {
  return useQuery<RepositoriesData, Error>({
    ...
    queryFn: () => getGithubFetcher<RepositoriesData, RepositoriesVariables>(
...
export const useRepositoryIssues = (owner: string, name: string, limit: number = 20) => {
  return useQuery<RepositoryIssuesData, Error>({
    queryKey: [QUERY_KEYS.ISSUES, owner, name, limit],
    queryFn: () => getGithubFetcher<RepositoryIssuesData, RepositoryIssuesVariables>(
